<template>
	<a-checkbox-group v-if="tagType == 'checkbox'" @change="onChange" :value="arrayValue" :disabled="disabled">
		<a-checkbox v-for="(item, key) in dictOptions" :key="key" :value="item.itemValue">
	...
	<a-select
		...
		allowClear
	>
		<a-select-option v-for="(item, index) in dictOptions" :key="index" :value="item.itemValue">
			<span style="display: inline-block; width: 100%" :title="item.itemText || item.label">
				{{ item.itemText || item.label }}
			</span>
		</a-select-option>
	</a-select>
...
export default {
	...
	methods: {
		...
		filterOption(input, option) {
			return (
				option.componentOptions.children[0].children[0].itemText.toLowerCase().indexOf(input.toLowerCase()) >= 0
			)
		},
