		...
		<header>
			...
			<a-row :gutter="[8, 14]" style="margin: 14px 0">
				<a-col :span="24">
					...
					<a-button @click="refreshDict" type="primary" icon="sync" style="margin-left: 8px">
						刷新字典缓存
					</a-button>
					<a-button @click="refreshCity" type="primary" icon="sync" style="margin-left: 8px">
						刷新区域缓存
					</a-button>
				</a-col>
			</a-row>
...
import { refreshDictCache, deleteDictItem } from '@/api/system'
import { refreshCityCache } from '@/api/common'
const columns = [
	{
		title: '序号',
		dataIndex: '',
	...
	methods: {
		...
		refreshDict() {
			refreshDictCache()
				.then((res) => {
					if (res.success) {
						this.$message.success('刷新字典缓存完成！')
					}
				})
				.catch((e) => {
					this.$message.warn('刷新字典缓存失败！')
				})
		},
		// 刷新区域缓存
		refreshCity() {
			refreshCityCache()
				.then((res) => {
					if (res.success) {
						this.$message.success('刷新区域缓存完成！')
					}
				})
				.catch((e) => {
					this.$message.warn('刷新区域缓存失败！')
				})
		},
	},
}
