		...
		<main>
			<a-form-model ref="form" :model="form" :rules="rules" :labelCol="labelCol" :wrapperCol="wrapperCol">
				<a-form-model-item label="菜单级别">
					<a-radio-group @change="onChangeMenuType" v-model="form.menuType" :disabled="disableSubmit">
				...
				<a-form-model-item label="所属子系统" :required="true" prop="subsystemId">
					<a-radio-group v-model="form.subsystemId" :disabled="disableSubmit">
						<a-radio :value="item.id" :key="index" v-for="(item, index) of subSystems">
							{{ item.name }}
						</a-radio>
					</a-radio-group>
				</a-form-model-item>
...
export default {
	...
	data() {
		return {
			...
			rules: {
				...
				perms: [{ trigger: 'blur', validator: this.validatePerms }],
				platformType: [{ required: true, message: '请选择平台菜单类型!' }],
				subsystemId: [{ required: true, message: '请选择子系统!' }],
			},
	...
	created() {
		this.getSubSystems()
		console.log('subSystems', this.subSystems)
	},
	...
	methods: {
		...
		handleSave() {
			let needValidates = []
			console.log('localMenuType', this.localMenuType)
			switch (this.localMenuType) {
				case '0':
					needValidates = ['name', 'url', 'component', 'platformType', 'subsystemId']
					break
				case '1':
					needValidates = ['name', 'parentId', 'url', 'component', 'platformType', 'subsystemId']
					break
				case '2':
					needValidates = ['name', 'parentId', 'permsType', 'platformType', 'subsystemId']
					break
			}
			...
			let validateResult = () => {
				return new Promise((resolve, reject) => {
					let flag = 1
					this.$refs['form'].validateField(needValidates, (errors, values) => {
						if (errors) {
							flag = 0
							return
						}
